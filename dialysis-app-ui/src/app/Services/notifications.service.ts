...
export class NotificationsService {
    ...
    public triggerNotificationReload$ = new Subject<void>();
    ...
    /** Call this when a reload should happen */
    triggerReload() {
        this.triggerNotificationReload$.next();
    }
    ...
    updateNotifications(notifications: any, userId?: number): Observable<any> {
        ...
        const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
        const url = userId
            ? `${this.apiBaseUrl}/notifications?user_id=${userId}`
